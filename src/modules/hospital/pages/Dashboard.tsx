...
import { Clock, MapPin, Truck, AlertCircle, Zap, ClipboardList, CheckCircle2, Wallet, HeartPulse, MoreHorizontal } from 'lucide-react';
        ...
        <div className="space-y-6 font-['Montserrat']">
            ...
            <div className="flex justify-between items-center bg-transparent py-2">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Welcome back, Dr. Adeyemi</p>
                </div>
                <div className="flex gap-4">
                    <button className="bg-[#E63946] hover:bg-red-700 text-white px-5 py-2.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors uppercase tracking-wide">
                        <AlertCircle size={16} /> Emergency Alert
                    </button>
                    <button className="bg-black hover:bg-gray-800 text-white px-5 py-2.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors uppercase tracking-wide">
                        <Zap size={16} /> Request Blood
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {metrics.map((m, i) => (
                    <div key={i} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between h-36 relative overflow-hidden">
                        <div className="absolute top-6 right-6 text-[10px] font-bold text-blue-500 uppercase tracking-widest bg-gray-50 px-2 py-1 rounded-sm">
                            {m.trend}
                        </div>
                        <div className={`w-10 h-10 rounded-lg ${m.bg} ${m.color} flex items-center justify-center mb-4`}>
                            <m.icon size={20} />
                        </div>
                        <div>
                            <p className="text-gray-500 text-xs font-bold mb-1">{m.label}</p>
                            <h3 className="text-3xl font-bold text-gray-900">
                                {m.value} <span className="text-xs text-gray-400 font-normal ml-1">{m.unit ? m.unit : ''}</span>
                            </h3>
                        </div>
                    </div>
                ))}
                                    ...
                                    <tr key={i} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                {req.urgency === 'CRITICAL' && <div className="w-2 h-2 bg-red-500 rounded-full"></div>}
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col">
                    ...
                    <div className="p-0 flex-1">
                        {nearbyInventory.map((bank, i) => (
                            <div key={i} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 group">
                    ...
                    <div className="p-4 bg-gray-100 rounded-b-xl border-t border-gray-100 text-center">
                        <button className="text-[10px] font-bold text-gray-500 hover:text-black uppercase tracking-widest">View Map View</button>
                    </div>
                </div>
            </div>
